...
const inferDbAction = (req: any): "read" | "write" | null => {
  const path = String(req.path || req.originalUrl || "").toLowerCase();
  if (path.includes("/db/query")) return "read";
  if (path.includes("/db/insert") || path.includes("/db/update") || path.includes("/db/delete")) return "write";
  return null;
};
...
const extractUserRolesFromMetadata = (metadata: any): string[] => {
  const roles = Array.isArray(metadata?.roles) ? metadata.roles : [];
  const role = metadata?.role ? [metadata.role] : [];
...
const writeAuditEntry = async (entry: Record<string, unknown>) => {
  try {
...
export const appendDeniedAccessAudit = async (req: any, error: any) => {
...
export const appendActionAudit = async (
  req: any,
  ...
  if (SKIP_AUDIT.has(table)) return;
  ...
  await writeAuditEntry({
    ...
    affected_ids: (after ?? []).map((r: any) => r?.id).filter(Boolean),
