...
const nextSequence = async (scope: string) => {
...
const nextSerial = async (prefix: string, scope = prefix) => {
  const sequence = await nextSequence(scope);
  return `${prefix}-${String(sequence).padStart(4, "0")}`;
};

const nextDailyNumber = async (prefix: string) => {
...
export const prepareInsertDocument = async (collection: string, rawInput: Record<string, unknown>) => {
  const now = nowDate();
  const doc = sanitizeDocument({ ...rawInput }) as Record<string, unknown>;
  ...
  if (collection === "timesheets") {
    ...
    if (doc.start_time && doc.end_time) {
      const [sh, sm] = (doc.start_time as string).split(":").map(Number);
      const [eh, em] = (doc.end_time as string).split(":").map(Number);
  ...
  if (collection === "storage_condition_readings") {
    ...
    if (doc.reading_key == null || doc.reading_key === "") delete doc.reading_key;
    else doc.reading_key = String(doc.reading_key);
