...
const computeOvertime = (hoursWorked: number) => {
  if (hoursWorked <= 8) return { regular: hoursWorked, overtime: 0, extraHours: 0 };
  if (hoursWorked <= 10) return { regular: 8, overtime: hoursWorked - 8, extraHours: 0 };
  return { regular: 8, overtime: 2, extraHours: hoursWorked - 10 };
};

const workDaysInMonth = (year: number, month: number): number => {
  let count = 0;
...
export class HrService {
  ...
  async getEmployeeSummary(employeeId: string, year: number, month: number) {
    ...
    const employee = sanitizeDocument(
      await Employees().findOne({ id: employeeId }).lean().exec(),
    ) as Record<string, unknown> | null;

    const tasks = sanitizeDocument(
      await Tasks().find({ assigned_to: employeeId }).lean().exec(),
    ) as Array<Record<string, unknown>>;
    ...
    for (const sheet of sheets) {
      ...
      if (sheet.work_date) workDates.add(String(sheet.work_date));
    }
  ...
  async getTeamMonthlySummary(year: number, month: number) {
    const startDate = `${year}-${String(month).padStart(2, "0")}-01`;
    const endDate = `${year}-${String(month).padStart(2, "0")}-31`;

    const allSheets = sanitizeDocument(
      await Timesheets()
    ...
    const allEmployees = sanitizeDocument(
      await Employees().find({ status: "active" }).lean().exec(),
    ) as Array<Record<string, unknown>>;

    // Group timesheets by employee
    const byEmployee: Record<string, Array<Record<string, unknown>>> = {};
    ...
    const perEmployee = allEmployees.map((emp) => {
      ...
      for (const s of sheets) {
        ...
        if (s.work_date) dates.add(String(s.work_date));
      }
  ...
  async getTeamPresenceToday() {
    ...
    const sheets = sanitizeDocument(
      await Timesheets().find({ work_date: today }).lean().exec(),
    ) as Array<Record<string, unknown>>;

    const activeEmployees = sanitizeDocument(
      await Employees().find({ status: "active" }).lean().exec(),
    ) as Array<Record<string, unknown>>;
  ...
  computeHours(startTime: string, endTime: string | null, breakMinutes: number): number | null {
