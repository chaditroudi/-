...
import {
  ...
  findFinanceApprovers,
  findPurchasingApprovers,
} from "../org/membership.js";
import type { WorkflowDecision } from "./purchase-requisition-workflow.js";

type RequisitionNotifyContext = {
  id: string;
...
const actionUrlFor = (id: string) => `/purchasing?requisition=${encodeURIComponent(id)}`;
...
export const notifyPurchaseRequisitionTransition = async (
  ...
  if (decision.notify.nextApproverLevel) {
    const targetUserIds = await resolveNextApproverIds(
      decision.notify.nextApproverLevel,
      department,
    );
    // Exclude the actor and the requester from the actionable queue.
    const filtered = targetUserIds.filter(
      (id) => id !== actorId && id !== requesterId,
    );
