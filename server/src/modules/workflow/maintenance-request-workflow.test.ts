...
} from "./maintenance-request-workflow.js";
...
describe("maintenance request workflow", () => {
  ...
  it("routes dept approval to MAINTENANCE_REVIEW then maintenance manager to APPROVED under threshold", () => {
    const afterDept = approveMaintenanceRequest(
    ...
    const afterMaint = approveMaintenanceRequest(
      baseRequest({
  ...
  it("adds a finance step when the estimated cost crosses the threshold", () => {
    const afterDept = approveMaintenanceRequest(
      baseRequest({ status: "SUBMITTED", estimated_cost: 8000 }),
      productionManager,
    );
    expect(afterDept.status).toBe("MAINTENANCE_REVIEW");
    expect(afterDept.notify.nextApproverLevel).toBe("maintenance_manager");
    ...
    const afterDaf = approveMaintenanceRequest(
      baseRequest({
  ...
  it("collapses the dept-manager level for maintenance-originated requests", () => {
    const decision = submitMaintenanceRequest(
      baseRequest({ department: "maintenance", requester_id: maintenanceTech.id }),
      maintenanceTech,
    );
  ...
  it("records rejection with the session actor identity", () => {
    const decision = rejectMaintenanceRequest(
      baseRequest({ status: "SUBMITTED" }),
      productionManager,
      "Budget insuffisant ce trimestre",
    );
  ...
  it("lets the requester cancel and maintenance close an approved request", () => {
    const cancelled = cancelMaintenanceRequest(
      baseRequest({ status: "SUBMITTED" }),
      productionEmployee,
      "Plus nécessaire",
    );
