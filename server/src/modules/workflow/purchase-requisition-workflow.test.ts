...
import {
  type PurchaseRequisitionWorkflowState,
...
const nextState = (
  base: PurchaseRequisitionWorkflowState,
  decision: WorkflowDecision,
): PurchaseRequisitionWorkflowState => ({
  ...base,
...
describe("purchase requisition escalation by amount", () => {
  const requisition = (estimated_cost: number): PurchaseRequisitionWorkflowState => ({
    status: "SUBMITTED",
    department: "stock",
    requester_id: employee.id,
    requester_name: "Ali Stock",
  ...
  it("escalates to the DAF above 10 000 and stops there under 50 000", () => {
    ...
    const afterPurchasing = approveRequisitionStep(
      nextState(base, afterDept),
      purchasingManager,
    );
    expect(afterPurchasing.status).toBe("FINANCE_APPROVAL");
    expect(afterPurchasing.notify.nextApproverLevel).toBe("daf");

    const afterFinance = approveRequisitionStep(
      nextState(base, afterPurchasing),
      financeDirector,
    );
  ...
  it("requires general direction above 50 000", () => {
    ...
    const afterDirection = approveRequisitionStep(
      nextState(base, afterFinance),
      generalDirection,
    );
  ...
  it("keeps the buyer out of the manager level", () => {
    ...
    expect(() => approveRequisitionStep(nextState(requisition(2500), afterDept), buyer)).toThrow(
      AppError,
    );
  });
  ...
  it("refuses a second signature from the same approver", () => {
    ...
    const afterDept = approveRequisitionStep(base, stockManager);
    expect(() =>
      approveRequisitionStep(nextState(base, afterDept), stockManager),
    ).toThrow(AppError);
  });
...
describe("purchase requisition exit branches", () => {
  ...
  it("clears prior signatures when returned for changes", () => {
    ...
    const resubmitted = submitRequisition(
      { ...submitted, status: decision.status, approvals: decision.approvals },
      employee,
    );
  ...
  it("lets the requester cancel but not once ordered", () => {
    ...
    expect(() => cancelRequisitionStep({ ...submitted, status: "ORDERED" }, employee)).toThrow(
      AppError,
    );
  });
