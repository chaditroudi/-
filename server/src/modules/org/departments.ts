...
const DEPARTMENT_ALIASES: Record<string, DepartmentCode> = {
  stock: "stock",
  magasin: "stock",
  stockage: "stock",
  warehouse: "stock",
...
export const isDepartmentCode = (value: string): value is DepartmentCode =>
  (DEPARTMENTS as readonly string[]).includes(value);

export const normalizeDepartment = (value: unknown): DepartmentCode | null => {
  if (typeof value !== "string") return null;
  const raw = value.trim().toLowerCase();
  if (!raw) return null;
  if (isDepartmentCode(raw)) return raw;
...
export const ROLE_DEFAULT_DEPARTMENT: Record<string, DepartmentCode> = {
