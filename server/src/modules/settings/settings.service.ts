...
const mergeSection = (defaults: Record<string, unknown>, value?: Record<string, unknown> | null) => ({
  ...defaults,
  ...(value || {}),
});

const mergeP2PSettings = (value?: Record<string, any> | null) => ({
  ...
  approval_matrix: Array.isArray(value?.approval_matrix) && value.approval_matrix.length > 0
    ? value.approval_matrix.map((step: Record<string, unknown>) => ({
        level: step.level,
...
export class SettingsService {
  ...
  async updateSettings(patch: Record<string, unknown>, updatedBy: string | null) {
    const current = await this.getSettings();
    const { id: _id, ...safeUpdate } = patch as any;
    ...
    const { _id: __id, ...rest } = updated as any;
    return normalizeSettings(rest);
  }
