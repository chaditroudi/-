...
const SITE_CODES = ["ROYALE_PALM", "ECODATTE", "BIOSCHA"] as const;
type P2PSiteCode = (typeof SITE_CODES)[number];
...
type RuntimeP2PSettings = {
  ...
  p2p: {
    ...
    sites: Record<P2PSiteCode, SiteRule>;
  };
};

const DEFAULT_SITE_RULES: Record<P2PSiteCode, SiteRule> = {
  ROYALE_PALM: {
    code: "ROYALE_PALM",
...
const normalizeSiteCode = (value: unknown): P2PSiteCode | null => {
  const normalized = String(value || "").trim().toUpperCase();
  return SITE_CODES.includes(normalized as P2PSiteCode) ? (normalized as P2PSiteCode) : null;
};
...
export class P2PService {
  private async getRuntimeSettings(): Promise<RuntimeP2PSettings> {
    const raw = (await SiteSettingsModel.findOne({ id: "global" })
      .select("operations p2p")
      .lean()
      .exec()) as Record<string, any> | null;

    const sites = SITE_CODES.reduce<Record<P2PSiteCode, SiteRule>>((acc, code) => {
      acc[code] = {
        ...DEFAULT_SITE_RULES[code],
        ...(raw?.p2p?.sites?.[code] || {}),
        cert_alert_days: Array.isArray(raw?.p2p?.sites?.[code]?.cert_alert_days)
          ? raw?.p2p?.sites?.[code]?.cert_alert_days.map((value: unknown) => Number(value)).filter((value: number) => Number.isFinite(value) && value > 0)
          : DEFAULT_SITE_RULES[code].cert_alert_days,
      };
      return acc;
    }, {} as Record<P2PSiteCode, SiteRule>);
  ...
  private async getRfqBudgetAmount(rfq: Record<string, unknown>) {
    const requisitionId = String(rfq.requisition_id || "").trim();
    if (requisitionId) {
      const requisition = sanitizeDocument(
    ...
    const responses = sanitizeDocument(
      await RFQResponses().find({ rfq_id: String(rfq.id || "") }).lean().exec(),
    ) as Array<Record<string, unknown>>;
    return responses.reduce((max, response) => Math.max(max, readNumber(response.total_amount)), 0);
  }

  private async ensureSupplierCertificateCompliance(site: P2PSiteCode | null, supplierId: string) {
    if (!site || !supplierId) return;

    const settings = await this.getRuntimeSettings();
    const siteRule = settings.p2p.sites[site];
    if (!siteRule.bio_certification_required) return;

    const certificates = sanitizeDocument(
      await SupplierCertificates().find({
    ...
    if (!hasValidCertificate) {
      throw badRequest(
        "SUPPLIER_CERTIFICATE_REQUIRED",
        `Le fournisseur sélectionné doit disposer d'un certificat bio valide pour ${siteRule.name}.`,
      );
    }
  }

  private async enrichPurchaseOrderContext(payload: Record<string, unknown>) {
    const purchaseOrderId = String(payload.purchase_order_id || "").trim();
    if (!purchaseOrderId) return payload;

    const purchaseOrder = sanitizeDocument(
  ...
  // ── RFQ ──────────────────────────────────────────────────────────────────────
  async listRFQs(status?: string) {
    const query = status ? { status } : {};
    return sanitizeDocument(
      await RFQRequests().find(query).sort({ created_at: -1 }).lean().exec(),
    );
  }

  async getRFQById(id: string) {
    const doc = sanitizeDocument(
      await RFQRequests().findOne({ id }).lean().exec(),
    ) as Record<string, unknown> | null;
    return doc;
  }

  async createRFQ(payload: Record<string, unknown>) {
  ...
  async updateRFQ(id: string, payload: Record<string, unknown>) {
    const existing = await RFQRequests().findOne({ id }).lean().exec();
    if (!existing) throw notFound("RFQ_NOT_FOUND", "Appel d'offres introuvable.");
    await RFQRequests().updateOne(
      { id },
      { $set: { ...cleanPatch(payload, ["id", "rfq_number", "created_at"]), updated_at: new Date().toISOString() } },
    ).exec();
  ...
  async getRFQResponses(rfqId: string) {
    return sanitizeDocument(
      await RFQResponses().find({ rfq_id: rfqId }).sort({ received_at: -1 }).lean().exec(),
    );
  }

  async addRFQResponse(rfqId: string, payload: Record<string, unknown>) {
    const rfq = await RFQRequests().findOne({ id: rfqId }).lean().exec();
    if (!rfq) throw notFound("RFQ_NOT_FOUND", "Appel d'offres introuvable.");
  ...
  async selectRFQWinner(rfqId: string, supplierId: string, reason: string) {
    if (!supplierId) throw badRequest("SUPPLIER_REQUIRED", "supplierId requis.");
    if (!reason) throw badRequest("REASON_REQUIRED", "reason requis.");

    const rfq = sanitizeDocument(
      await RFQRequests().findOne({ id: rfqId }).lean().exec(),
    ) as Record<string, unknown> | null;
    if (!rfq) throw notFound("RFQ_NOT_FOUND", "Appel d'offres introuvable.");

    const responses = sanitizeDocument(
      await RFQResponses().find({ rfq_id: rfqId }).lean().exec(),
    ) as Array<Record<string, unknown>>;
    ...
    if (
      ...
      throw badRequest(
        "RFQ_MINIMUM_QUOTES_REQUIRED",
        `Au moins ${settings.p2p.rfq_minimum_quote_count} devis sont requis au-delà de ${settings.p2p.rfq_quote_threshold_tnd} TND.`,
      );
    }
  ...
  // ── Goods Receipts ────────────────────────────────────────────────────────────
  async listGoodsReceipts(params?: { status?: string; supplier_id?: string }) {
    const query: Record<string, unknown> = {};
    if (params?.status) query.status = params.status;
    if (params?.supplier_id) query.supplier_id = params.supplier_id;
    return sanitizeDocument(
      await GoodsReceipts().find(query).sort({ created_at: -1 }).lean().exec(),
    );
  }

  async getGoodsReceiptById(id: string) {
    return sanitizeDocument(
      await GoodsReceipts().findOne({ id }).lean().exec(),
    ) as Record<string, unknown> | null;
  }

  async createGoodsReceipt(payload: Record<string, unknown>) {
    const settings = await this.getRuntimeSettings();
    const siteCode = normalizeSiteCode(payload.site);
    const siteRule = siteCode ? settings.p2p.sites[siteCode] : null;

    const requirePurchaseOrder =
    ...
    const forcedQuarantineReason =
      siteRule?.systematic_quarantine && !String(enrichedPayload.quarantine_reason || "").trim()
  ...
  async updateGoodsReceipt(id: string, payload: Record<string, unknown>) {
    const existing = await GoodsReceipts().findOne({ id }).lean().exec();
    if (!existing) throw notFound("GOODS_RECEIPT_NOT_FOUND", "Bon de réception introuvable.");
    await GoodsReceipts().updateOne(
      { id },
      { $set: { ...cleanPatch(payload, ["id", "receipt_number", "created_at"]), updated_at: new Date().toISOString() } },
    ).exec();
    return this.getGoodsReceiptById(id);
  }

  async releaseQuarantine(id: string, decision: string, by: string, notes?: string) {
    if (!decision) throw badRequest("DECISION_REQUIRED", "decision requis (ACCEPTE ou REFUSE).");
    if (!by) throw badRequest("BY_REQUIRED", "by requis.");
  ...
  // ── Supplier Invoices ─────────────────────────────────────────────────────────
  async listInvoices(params?: { status?: string; supplier_id?: string }) {
    const query: Record<string, unknown> = {};
    if (params?.status) query.status = params.status;
    if (params?.supplier_id) query.supplier_id = params.supplier_id;
    return sanitizeDocument(
      await SupplierInvoices().find(query).sort({ created_at: -1 }).lean().exec(),
    );
  }

  async getInvoiceById(id: string) {
    return sanitizeDocument(
      await SupplierInvoices().findOne({ id }).lean().exec(),
    ) as Record<string, unknown> | null;
  }
  ...
  async updateInvoice(id: string, payload: Record<string, unknown>) {
    ...
    if (!existing) throw notFound("INVOICE_NOT_FOUND", "Facture introuvable.");
    await SupplierInvoices().updateOne(
      { id },
      { $set: { ...cleanPatch(payload, ["id", "invoice_number", "created_at"]), updated_at: new Date().toISOString() } },
    ).exec();
    return this.getInvoiceById(id);
  }

  async runThreeWayMatch(invoiceId: string, tolerancePct?: number) {
    const settings = await this.getRuntimeSettings();
    const invoice = sanitizeDocument(
      await SupplierInvoices().findOne({ id: invoiceId }).lean().exec(),
    ) as Record<string, unknown> | null;

    if (!invoice) throw notFound("INVOICE_NOT_FOUND", "Facture introuvable.");
    ...
    if (ref === 0) {
      ...
      if (variancePct === 0) matchResult = "MATCH";
      else if (variancePct <= effectiveTolerance) matchResult = "ECART_TOLERANCE";
      else matchResult = "ECART_BLOQUANT";
    }
  ...
  async approvePayment(invoiceId: string, by: string) {
    if (!by) throw badRequest("BY_REQUIRED", "by requis.");
    const settings = await this.getRuntimeSettings();
    ...
    if (invoice.match_result === "ECART_BLOQUANT") {
      throw badRequest(
        "PAYMENT_BLOCKED",
        "Paiement bloqué — écart de rapprochement supérieur à la tolérance autorisée.",
      );
    }
    if (!["MATCH", "ECART_TOLERANCE"].includes(String(invoice.match_result || ""))) {
      throw badRequest(
        "THREE_WAY_MATCH_REQUIRED",
        "Le rapprochement 3 voies doit être validé avant le bon à payer.",
      );
    }
    if (
      settings.p2p.require_goods_receipt_for_payment &&
      !String(invoice.goods_receipt_id || "").trim() &&
      readNumber(invoice.br_amount) <= 0
    ) {
      throw badRequest(
        "GOODS_RECEIPT_REQUIRED",
        "Une référence de bon de réception est requise avant le bon à payer.",
      );
    }
  ...
  async markPaid(invoiceId: string, paymentRef: string, by: string) {
    if (!paymentRef) throw badRequest("PAYMENT_REF_REQUIRED", "paymentRef requis.");
    if (!by) throw badRequest("BY_REQUIRED", "by requis.");
    const now = new Date().toISOString();
    return this.updateInvoice(invoiceId, {
      status: "PAYEE",
  ...
  async listCertificates(params?: { supplier_id?: string; type?: string }) {
    const query: Record<string, unknown> = {};
    if (params?.supplier_id) query.supplier_id = params.supplier_id;
    if (params?.type) query.certificate_type = params.type;
    return sanitizeDocument(
      await SupplierCertificates().find(query).sort({ expiry_date: 1 }).lean().exec(),
    );
  }

  async createCertificate(payload: Record<string, unknown>) {
    if (!payload.supplier_id) throw badRequest("SUPPLIER_REQUIRED", "supplier_id requis.");
    if (!payload.expiry_date) throw badRequest("EXPIRY_REQUIRED", "expiry_date requis.");
  ...
  async updateCertificate(id: string, payload: Record<string, unknown>) {
    const existing = await SupplierCertificates().findOne({ id }).lean().exec();
    if (!existing) throw notFound("CERT_NOT_FOUND", "Certificat introuvable.");
    await SupplierCertificates().updateOne(
      { id },
      { $set: { ...cleanPatch(payload, ["id", "created_at"]), updated_at: new Date().toISOString() } },
    ).exec();
  ...
  async deleteCertificate(id: string) {
    ...
    if (!result.deletedCount) throw notFound("CERT_NOT_FOUND", "Certificat introuvable.");
    return { id, deleted: true };
  }

  // ── Budget Centers ─────────────────────────────────────────────────────────────
  async listBudgetCenters(site?: string) {
    const query = site ? { site } : {};
    return sanitizeDocument(
      await BudgetCenters().find(query).sort({ code: 1 }).lean().exec(),
    );
  }

  async createBudgetCenter(payload: Record<string, unknown>) {
    if (!payload.code) throw badRequest("CODE_REQUIRED", "code requis.");
  ...
  async updateBudgetCenter(id: string, payload: Record<string, unknown>) {
    const existing = await BudgetCenters().findOne({ id }).lean().exec();
    if (!existing) throw notFound("BUDGET_CENTER_NOT_FOUND", "Centre budgétaire introuvable.");
    await BudgetCenters().updateOne(
      { id },
      { $set: { ...cleanPatch(payload, ["id", "code", "created_at"]), updated_at: new Date().toISOString() } },
    ).exec();
