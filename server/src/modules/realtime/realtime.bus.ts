...
export type RealtimeDbAction = "INSERT" | "UPDATE" | "DELETE" | "UPSERT" | "SYNC";
...
const COLLECTION_TO_RESOURCE: Record<string, string> = {
...
const resolveResource = (table: string, explicitResource?: string): string =>
  explicitResource || COLLECTION_TO_RESOURCE[table] || table;

export type RealtimeDbChangeEvent = {
  type?: string;
  table: string;
  resource?: string; // explicit override; auto-derived from table if omitted
  action: RealtimeDbAction;
  actorId?: string | null;
  rowIds?: string[];
  rows?: unknown[];
  before?: unknown[];
...
export const addRealtimeClient = (client: RealtimeClient) => {
  clients.set(client.id, client);
};

export const removeRealtimeClient = (clientId: string) => {
...
export const publishRealtimeDbChange = (event: RealtimeDbChangeEvent) => {
