...
export class PackagingService {
  ...
  async closeOrder(id: string, payload: Record<string, unknown>) {
    ...
    const sourceLot = sanitizeDocument(
      await StockLots()
        .findOne({
    ...
    const bom = existing.bom_id
      ? (sanitizeDocument(
          await PackagingBoms().findOne({ id: existing.bom_id }).lean().exec(),
        ) as Record<string, unknown> | null)
      : null;
    ...
    const costTndPerKg =
      producedKg > 0 ? Number((absorbed.totalCostTnd / producedKg).toFixed(4)) : null;
  ...
  async sealPalette(id: string, payload: Record<string, unknown>) {
    ...
    try {
      const order = sanitizeDocument(
        await PackagingOrders().findOne({ id: orderId }).lean().exec(),
      ) as PackagingOrderRow | null;
      const sourceLotNumber = readString(order?.source_lot_number);
      let parentLotId: string | null = null;

      if (order?.source_sublot_id) {
        const sublot = sanitizeDocument(
          await TriageSublots().findOne({ id: order.source_sublot_id }).lean().exec(),
        ) as TriageSubLotRow | null;
