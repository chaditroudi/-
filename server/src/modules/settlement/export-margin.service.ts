...
const loadFxRates = async (): Promise<FxRateBook> => {
  ...
  try {
    const doc = (await SiteSettingsModel.findOne({ id: "global" })
      ...
      .exec()) as {
      costing?: {
        fx_eur_to_tnd?: number | null;
        fx_usd_to_tnd?: number | null;
        fx_sar_to_tnd?: number | null;
      };
    } | null;
...
@Injectable()
export class ExportMarginService {
  private async resolveLineCostTndPerKg(line: Record<string, unknown>): Promise<{
    costTndPerKg: number | null;
    ...
    if (lotId || lotRef) {
      ...
      const receptionLot = sanitizeDocument(
        await ReceptionLots()
          .findOne({
            $or: [
      ...
      if (parentInternal) {
        const parent = sanitizeDocument(
          await ReceptionLots().findOne({ lot_internal: parentInternal }).lean().exec(),
        ) as Record<string, unknown> | null;
  ...
  async getOrderMargin(orderId: string) {
    const order = sanitizeDocument(
      await ExportOrders().findOne({ id: orderId }).lean().exec(),
    ) as Record<string, unknown> | null;
    if (!order) throw notFound("EXPORT_ORDER_NOT_FOUND", "Commande export introuvable.");
    ...
    const revenueInvoice =
      Number(order.total_amount) > 0
        ? Number(order.total_amount)
        : lines.reduce(
            (sum, line) => sum + Number(line.net_weight_kg || 0) * Number(line.unit_price || 0),
            0,
          );
    ...
    for (const line of lines) {
      ...
      const lineCost =
        resolved.costTndPerKg != null && weightKg > 0
