...
import { verifyLotEventChain, type LotEventRecord } from "../trust/lot-ledger.js";
...
type SupplierQuery = { $or?: Array<{ id?: string; code?: string }> };
const created: Record<string, TestRow[]> = {};
const receptions = new Map<string, TestRow>();
...
vi.mock("../../db/dynamic-model.js", () => ({
  ...
  getCollectionModel: (collection: string) => {
    if (collection === "suppliers") {
      return {
        findOne: vi.fn((query: SupplierQuery) => ({
          lean: () => ({
            exec: async () => {
    ...
    if (collection === "receptions_v2") {
      return {
        findOne: vi.fn((query: { $or?: Array<{ id?: string }> }) => ({
          lean: () => ({
            exec: async () => {
              const clauses = query?.$or || [];
        ...
        create: vi.fn(async (rows: TestRow[]) => {
          created[collection] = [...(created[collection] || []), ...rows];
          rows.forEach((row) => {
            if (row.id) receptions.set(String(row.id), row);
          });
        }),
...
vi.mock("./reception-stock-sync.js", () => ({
  ...
  loadReceptionLots: vi.fn(async (receptionId: string) =>
    (created.reception_lots || []).filter((lot) => lot.reception_id === receptionId),
  ),
}));
...
describe("receptionsService", () => {
  ...
  it("appends a verifiable SUPPLIER_INTAKE ledger event for every lot created at intake", async () => {
    ...
    const events = (created.lot_events || []) as unknown as LotEventRecord[];
    ...
    const w1Failures = ledgerFailure.mock.calls.filter((call) =>
      String(call[0] ?? "").includes("lot lifecycle recordStage failed"),
    );
  ...
  it("bootstraps intake and appends QC_DECIDED to the ledger when QC accepts a lot", async () => {
    ...
    await receptionsService.submitQcDecision(
      { inspectionId: "insp-1", decision: "ACCEPTE", qualitySummary: { grade: "A" } },
      { id: "qc-1" },
    );
  ...
  it("appends REJECTED to the ledger when QC rejects a lot", async () => {
    ...
    await receptionsService.submitQcDecision(
      { inspectionId: "insp-2", decision: "REJETE", qualitySummary: { grade: "R" }, comment: "moisissure" },
      { id: "qc-2" },
    );
  ...
  it("appends WEIGHED after a verified gross+tare pair", async () => {
    ...
    await receptionsService.recordWeighing(
      "lot-w",
      { type: "GROSS", weight_kg: 1000, source: "MANUAL", manual_reason: "scale outage" },
      { id: "user-1" },
    );
    await receptionsService.recordWeighing(
      "lot-w",
      { type: "TARE", weight_kg: 100, source: "MANUAL", manual_reason: "scale outage" },
      { id: "user-1" },
    );
  ...
  it("blocks a non-admin from starting QC on their own reception", async () => {
    ...
    await expect(
      receptionsService.startQcInspection(
        {
          reception_id: "rec-1",
