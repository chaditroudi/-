...
export type FumigationReadingEvidence = {
  read_at?: string | null;
...
export type ColdReadingEvidence = {
...
export type ProtocolConfig = {
  ...
  min_avg_concentration_gm3?: number;
  max_external_leak_ppm?: number;
};

const avgConcentration = (reading: FumigationReadingEvidence) => {
...
export const digestReadings = (readings: Array<Record<string, unknown>>) => {
...
export const evaluateFumigationCcp = (input: {
  ...
  t0Start?: string | null;
  readings: FumigationReadingEvidence[];
  protocolConfig?: ProtocolConfig | null;
  now?: string;
}) => {
  const missing: string[] = [];
  const readings = [...input.readings].sort((a, b) =>
    String(a.read_at || "").localeCompare(String(b.read_at || "")),
  );
  if (readings.length === 0) missing.push("readings");
  if (!input.t0Start) missing.push("t0_start");
  ...
  const parametersCompliant =
    avgConcentrationValue != null && avgConcentrationValue >= minRequired;

  const leakEvents = readings.filter(
    (reading) => Number(reading.external_leak_ppm || 0) > (protocol.max_external_leak_ppm ?? 0.3),
  ).length;
  const doorBreaches = readings.filter((reading) => reading.door_locked === false).length;
  const deviceVerifiedCount = readings.filter(
    (reading) => reading.source === "DEVICE" && reading.signature_verified === true,
  ).length;

  // CT approximation: average concentration × elapsed hours while readings exist.
  const ctProduct =
    avgConcentrationValue != null && durationMinutes > 0
  ...
  return {
    ...
    doorBreaches,
    missing,
    readingsDigest: digestReadings(readings as Array<Record<string, unknown>>),
  };
};

export const evaluateColdChain = (input: {
  readings: ColdReadingEvidence[];
  ...
  return {
    ...
    deviceVerifiedCount: input.readings.filter(
      (reading) => reading.source === "DEVICE" && reading.signature_verified === true,
    ).length,
    readingsDigest: digestReadings(input.readings as Array<Record<string, unknown>>),
  };
};

export const ccpGateMode = (): "off" | "warn" | "enforce" => {
