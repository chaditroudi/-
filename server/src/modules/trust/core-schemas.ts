...
import type { CoreSchemaCollection } from "./regulated-collections.js";
...
export const CORE_SCHEMAS: Record<CoreSchemaCollection, z.ZodTypeAny> = {
...
export const validateCoreDocument = (collection: string, doc: Record<string, unknown>) => {
  const schema = CORE_SCHEMAS[collection as CoreSchemaCollection];
  if (!schema) return { ok: true as const, data: doc, errors: [] as string[] };

  const parsed = schema.safeParse(doc);
  if (parsed.success) {
    return { ok: true as const, data: parsed.data as Record<string, unknown>, errors: [] as string[] };
  }

  const errors = parsed.error.issues.map((issue) => `${issue.path.join(".") || "root"}: ${issue.message}`);
  return { ok: false as const, data: doc, errors };
};
