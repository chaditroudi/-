...
describe("W3 weighbridge signing", () => {
  const reading = {
    ...
    unit: "kg",
    stable: true,
    direction: "GROSS" as const,
  };

  it("produces a deterministic canonical frame and HMAC", () => {
    expect(canonicalReading(reading)).toBe(
      "SIM-01:R-1:2026-07-26T12:00:00.000Z:1250.500:kg:1:GROSS",
    );
  ...
  it("changes the signature when weight or secret changes", () => {
    expect(signReading(reading, "secret")).not.toBe(
      signReading({ ...reading, weightKg: 1250.6 }, "secret"),
    );
