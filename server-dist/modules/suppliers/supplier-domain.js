...
const requireDate = (value, code, message) => {
...
const assertDateOrder = (start, end, code = "INVALID_SUPPLIER_DATE_RANGE") => {
    if (!start || !end)
        return;
...
const normalizeContractRecords = (value) => {
    if (!Array.isArray(value))
        return [];
...
const contractLifecycleStatus = (contract, today = new Date()) => {
    ...
    if (Number.isNaN(end.getTime()))
        return contract.status;
    if (end.getTime() < today.setHours(0, 0, 0, 0))
        return "expired";
    return contract.status === "draft" ? "draft" : "active";
};
const computeComplianceStatus = (doc) => {
...
export const normalizeSupplierDocument = (input, existing) => {
    const doc = {
        ...(existing || {}),
        ...(input || {}),
    };
    ...
    const gradePricesRaw = doc.grade_prices_tnd_per_kg && typeof doc.grade_prices_tnd_per_kg === "object"
    ...
    doc.contract_start_date = requireDate(doc.contract_start_date, "CONTRACT_START_DATE_REQUIRED", "Contract start date is required.");
    doc.contract_end_date = requireDate(doc.contract_end_date, "CONTRACT_END_DATE_REQUIRED", "Contract end date is required.");
    assertDateOrder(doc.contract_start_date, doc.contract_end_date);
...
export const appendSupplierPerformanceSnapshot = (before, after, actorId) => {
    const tracked = [
        "quality_score",
    ...
    if (!changed)
        return Array.isArray(after.performance_history) ? after.performance_history : [];
