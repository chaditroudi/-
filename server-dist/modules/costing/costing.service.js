...
let CostingService = class CostingService {
    ...
    async getSummary(periodRaw) {
        ...
        const rates = await loadCostingRates();
        const triageSessions = sanitizeDocument(await TriageSessions()
            .find({
            status: "TERMINE",
            $or: [
            ...
            .lean()
            .exec()) || [];
        const packagingOrders = sanitizeDocument(await PackagingOrders()
            .find({
            status: "TERMINE",
            $or: [
                { ended_at: { $gte: startIso } },
            ...
            .lean()
            .exec()) || [];
        const productionOrders = sanitizeDocument(await ProductionOrders()
            .find({
            status: "COMPLETED",
            $or: [
            ...
            .lean()
            .exec()) || [];
        const receptionLots = sanitizeDocument(await ReceptionLots()
            .find({
        ...
        const pkgMaterial = sum(packagingOrders, "material_cost_tnd") + sum(packagingOrders, "packaging_cost_tnd");
        ...
        const energyCost = sum(triageSessions, "energy_cost_tnd") + sum(packagingOrders, "energy_cost_tnd");
        const producedKg = pkgKg > 0
            ? pkgKg
        ...
        const breakdown = [
            { category: "materials", label: "Matières premières", amount: materialCost },
            { category: "labor", label: "Main-d'œuvre (standard)", amount: labourCost },
            { category: "energy", label: "Énergie (standard)", amount: energyCost },
            { category: "overhead", label: "Frais généraux (standard)", amount: overheadCost },
            { category: "losses", label: "Pertes", amount: lossCost },
            { category: "maintenance", label: "Maintenance", amount: 0 },
        ].map((row) => ({
            ...row,
        ...
        return {
            ...
            totals: {
                ...
                cost_variance_pct: targetCostPerKg > 0
            ...
            kpis: {
                ...
                targetCostPerKg,
                costVariancePercent: targetCostPerKg > 0
                    ? Number((((costPerKg - targetCostPerKg) / targetCostPerKg) * 100).toFixed(2))
                    : 0,
                ...
                materialCostPerTon: producedKg > 0 ? Number(((materialCost / producedKg) * 1000).toFixed(2)) : 0,
                overheadCostPerTon: producedKg > 0 ? Number(((overheadCost / producedKg) * 1000).toFixed(2)) : 0,
