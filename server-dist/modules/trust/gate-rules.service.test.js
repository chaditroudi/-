...
vi.mock("../../db/defaults.js", () => ({
    prepareInsertDocument: vi.fn(async (_collection, value) => ({
...
const seedCompleteChain = (lotId) => {
    ...
    const packed = buildLotEventRecord({ lotId, eventType: "PACKAGED", collection: "packaging_palettes", action: "GATE", prevHash: ccp.hash }, 4);
    store.lot_events = [intake, qc, ccp, packed];
};
describe("gateRulesService.assertShipmentWritable", () => {
    ...
    it("allows non-closing statuses without reading the ledger", async () => {
        await expect(gateRulesService.assertShipmentWritable(null, { status: "DRAFT", lot_ids: ["lot-1"] })).resolves.toBeUndefined();
    ...
    it("blocks closure when the ledger dossier is incomplete and appends GATE_BLOCKED", async () => {
        const intake = buildLotEventRecord({ lotId: "lot-1", eventType: "LOT_CREATED", collection: "reception_lots", action: "GATE", prevHash: null }, 1);
        store.lot_events = [intake];
        await expect(gateRulesService.assertShipmentWritable(null, { status: "CLOSED", lot_ids: ["lot-1"] }, { collection: "shipment_preparations" })).rejects.toMatchObject({ code: "SHIPMENT_DOSSIER_INCOMPLETE" });
    ...
    it("allows closure when the ledger proves genealogy, QC, CCP and packaging", async () => {
        ...
        await expect(gateRulesService.assertShipmentWritable(null, { statut: "EXPEDIE", lignes: [{ reception_lot_id: "lot-1" }] }, { collection: "bon_expeditions" })).resolves.toBeUndefined();
    });
    it("ignores client dossier booleans and still blocks on an incomplete chain", async () => {
        store.lot_events = [];
        await expect(gateRulesService.assertShipmentWritable(null, {
            status: "SHIPPED",
            lot_ids: ["lot-missing"],
            qc_complete: true,
            ccp_complete: true,
