...
import { isAppendOnlyCollection, isCoreSchemaCollection, isRegulatedCollection, } from "./regulated-collections.js";
 ...
 */
export const assertGenericDbMutationAllowed = (table, action, options) => {
    ...
    if (isAppendOnlyCollection(table) && (action === "update" || action === "delete")) {
        throw badRequest("IMMUTABLE_LOG", `${table} is an append-only history log and cannot be ${action === "delete" ? "deleted" : "updated"}.`);
    }
    if (isRegulatedCollection(table) && action === "delete") {
        throw badRequest("REGULATED_DELETE_FORBIDDEN", `${table} is a regulated MES record and cannot be deleted via the generic API.`);
    }
};
export const assertCoreSchema = (table, docs) => {
    if (!isCoreSchemaCollection(table))
        return;
