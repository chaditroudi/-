...
import { getSwallowedStageFailureCount, lotLifecycleService, resetSwallowedStageFailureCount, } from "./lot-lifecycle.service.js";
const store = {};
const emitted = [];
...
vi.mock("../notifications/notification-emitter.js", () => ({
    emitNotification: vi.fn(async (input) => {
        emitted.push(input);
        return input;
    }),
}));
describe("lotLifecycleService.recordStageSafe", () => {
    ...
    it("appends SHIPPED when packing is already on the chain", async () => {
        ...
        let prev = null;
        const stages = ["SUPPLIER_INTAKE", "QC_DECIDED", "FUMIGATION_CCP", "TRIAGE", "PACKED"];
