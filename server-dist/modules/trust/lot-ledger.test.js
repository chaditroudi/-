...
import { buildLotEventRecord, computeLotEventHash, inferLotIdsFromRow, lotLedgerGenesisHash, verifyLotEventChain, } from "./lot-ledger.js";
describe("W0 lot event ledger", () => {
    ...
    it("detects tampering when a payload is altered after the fact", () => {
        ...
        expect(result.expectedHash).toBe(computeLotEventHash({
    ...
    it("infers lot ids from regulated row shapes", () => {
        ...
        expect(inferLotIdsFromRow("production_orders", {
            id: "PO1",
            input_lot_ids: ["A", "B"],
        })).toEqual(["A", "B"]);
        expect(inferLotIdsFromRow("stock_movements", {
            id: "M1",
            lot_id: "S1",
            reception_lot_id: "R1",
        })).toEqual(["S1", "R1"]);
