...
const compactRows = (items) => items.filter(Boolean);
const normalizeLotReference = (value) => {
...
const extractLotReferences = (value) => {
    const raw = String(value || "").trim();
    if (!raw)
        return [];
    ...
    try {
        ...
        [
            ...
            if (parsed[key])
                references.push(normalizeLotReference(parsed[key]));
        });
    }
    catch {
    ...
    try {
        ...
        [
            ...
            if (candidate)
                references.push(normalizeLotReference(candidate));
        });
    }
    catch {
        // Not a URL.
...
const normalizeMovementType = (value) => {
    ...
    const movementType = raw.toUpperCase();
...
const normalizeMovementReason = (value) => {
...
const parseRequiredPaletteQuantity = (value) => {
...
const getModelRowsByCode = async (collection, codes) => {
    const Model = getCollectionModel(collection);
    return sanitizeDocument(await Model.find({ code: { $in: codes } }).lean().exec());
};
const findLocation = async (payload) => {
    ...
    return sanitizeDocument(await Locations.findOne(query).lean().exec());
};
const findZone = async (payload) => {
...
const mapStorageAlertNotificationType = (alert) => {
...
const mapStorageAlertEntity = (alert) => {
    if (alert.lot_id) {
...
const createNotificationForStorageAlert = async (alert) => {
    const alertKey = String(alert.alert_key || "").trim();
    if (alertKey) {
        const existing = sanitizeDocument(await SystemNotifications().findOne({ "metadata.alert_key": alertKey }).lean().exec());
        if (existing)
            return existing;
    }
...
const ensureAlert = async (payload) => {
    ...
    if (alertKey) {
        ...
        if (existing) {
            return {
                alert: sanitizeDocument(existing),
                created: false,
            };
        }
    }
    ...
    return {
        alert: sanitizeDocument(prepared),
        created: true,
    };
};
const ensureAlertWithNotification = async (payload) => {
    ...
    const notification = result.created
        ...
        : null;
    return {
        alerts: result.created ? [result.alert] : [],
        notifications: compactRows([notification]),
    };
};
const suggestDestinationLocation = async (payload) => {
    const Locations = getCollectionModel("storage_locations");
    const zoneQuery = { is_active: true, location_status: { $ne: "blocked" } };
    if (payload.destinationZoneId)
        zoneQuery.storage_zone_id = payload.destinationZoneId;
    if (payload.destinationZoneCode)
        zoneQuery.zone_code = String(payload.destinationZoneCode).trim().toUpperCase();
    const candidates = sanitizeDocument(await Locations.find(zoneQuery).lean().exec());
...
const updateZoneLoad = async (zoneId, palettesDelta, kgDelta) => {
    if (!zoneId)
        return;
    const Zones = getCollectionModel("storage_zones");
    const zone = sanitizeDocument(await Zones.findOne({ id: zoneId }).lean().exec());
    if (!zone)
        return;
    await Zones.updateOne({ id: zoneId }, {
        $set: {
...
const resolveStockLotByRef = async (payload) => {
    ...
    const stockLot = sanitizeDocument(await StockLots.findOne({ $or: clauses }).lean().exec());
    if (stockLot)
        return stockLot;
    ...
    const unit = sanitizeDocument(await ReceptionUnits.findOne({ $or: unitClauses }).lean().exec());
    if (!unit?.reception_lot_id)
        return null;
    return sanitizeDocument(await StockLots.findOne({ reception_lot_id: unit.reception_lot_id }).lean().exec());
};
const getLotIdentifiers = (payload, lot) => unique([
    payload.lotId,
    payload.lotCode,
    lot?.id,
    lot?.lot_number,
...
const getFefoCandidates = async (payload) => {
    const StockLots = getCollectionModel("stock_lots");
    ...
    if (payload.productId)
        query.product_id = payload.productId;
    if (payload.variety)
        query.variety = payload.variety;
    const rows = sanitizeDocument(await StockLots.find(query).lean().exec());
    return sortLotsForFefo(rows).slice(0, Number(payload.limit || 50));
};
const validateFefoPicking = async (payload, lot) => {
...
const enforceVarietyAndBioRules = async (payload, destination, lot) => {
    ...
    const lotIsBio = Boolean(payload.lotIsBio ?? lot?.is_bio ?? lot?.bio_declared);
...
const syncLotMovementHistory = async (payload, lot, source, destination, quantityKg, movementDate, movementType, operatorId) => {
    if (!lot?.id || !lot.product_id)
        return;
...
let StorageService = class StorageService {
    async listModule3Zones() {
        ...
        const zones = sanitizeDocument(rawZones);
    ...
    async listModule3Locations(storageZoneId) {
        ...
        const [locations, zones, lotsByZoneAgg, lotInternalsAgg] = await Promise.all([
            sanitizeDocument(await getCollectionModel("storage_locations")
                .find(query)
                .sort({ code: 1 })
                .lean()
                .exec()),
        ...
        return locations.map((location) => {
            ...
            return {
                ...
                storage_zone: zonesById.get(String(location.storage_zone_id || "")) || null,
            };
        });
    }
    async listStorageConditionReadings(limit = 80) {
        ...
        const [rows, zones, locations] = await Promise.all([
            sanitizeDocument(await getCollectionModel("storage_condition_readings")
                .find({})
                .sort({ reading_at: -1 })
                .limit(safeLimit)
                .lean()
                .exec()),
            sanitizeDocument(await getCollectionModel("storage_zones").find({}).lean().exec()),
            sanitizeDocument(await getCollectionModel("storage_locations").find({}).lean().exec()),
        ]);
        const zonesById = new Map(zones.map((zone) => [String(zone.id || ""), zone]));
    ...
    async listStorageLocationMovements(limit = 80) {
        ...
        const [rows, locations] = await Promise.all([
            sanitizeDocument(await getCollectionModel("storage_location_movements")
                .find({})
    ...
    async listStorageDoorEvents(limit = 80) {
        const safeLimit = Math.min(Math.max(Number(limit || 80), 1), 500);
        return sanitizeDocument(await getCollectionModel("storage_door_events")
            .find({})
            .sort({ event_at: -1 })
            .limit(safeLimit)
            .lean()
    ...
    async listStorageDlcAlerts(limit = 15) {
        ...
        const [notifications, alerts] = await Promise.all([
            sanitizeDocument(await SystemNotifications()
                .find({
                is_read: { $ne: true },
    ...
    async seedModule3() {
        ...
        const zonesByCode = new Map(existingZones.map((zone) => [String(zone.code), zone]));
        const zonesToInsert = [];
        for (const seed of ROYAL_PALM_MODULE3_ZONES) {
            ...
            zonesToInsert.push(await prepareInsertDocument("storage_zones", {
                ...seed,
                current_load_kg: 0,
        ...
        const existingLocations = await getModelRowsByCode("storage_locations", locationSeeds.map((location) => location.code));
        ...
        for (const location of locationSeeds.filter((item) => existingLocationCodes.has(item.code))) {
            await Locations.updateOne({ code: location.code }, {
                $set: {
    ...
    async suggestLocation(rawPayload) {
        ...
        const Locations = getCollectionModel("storage_locations");
        const candidates = sanitizeDocument(await Locations.find({
    ...
    async recordDoorEvent(rawPayload, user) {
        const payload = rawPayload;
        const zone = await findZone({
        ...
        const recentEvents = sanitizeDocument(await DoorEvents.find({
            storage_zone_id: zone.id,
    ...
    async evaluateBusinessRules() {
        ...
        const createdAlerts = [];
        const createdNotifications = [];
        const createdCycleCounts = [];
        const lots = sanitizeDocument(await StockLots.find({ current_quantity: { $gt: 0 } }).lean().exec());
        ...
        const rawLocations = sanitizeDocument(await Locations.find({
            zone_code: /^SB-/,
            lot_ids_present: { $exists: true, $ne: [] },
        }).lean().exec());
        for (const location of rawLocations) {
            if (!isRawStorageDelayed(location, now))
                continue;
            rawDelayAlerts += 1;
            const result = await ensureAlertWithNotification({
        ...
        const zones = sanitizeDocument(await Zones.find({ is_active: true }).lean().exec());
        for (const zone of zones) {
            ...
            createdCycleCounts.push(sanitizeDocument(cycleCount));
    ...
    async recordReading(rawPayload, user) {
        const payload = rawPayload;
        const Zones = getCollectionModel("storage_zones");
        const Locations = getCollectionModel("storage_locations");
        ...
        if (payload.locationCode) {
            try {
                ...
                throw badRequest("INVALID_LOCATION_CODE", error instanceof Error ? error.message : "Invalid location code.");
            }
        }
        ...
        const result = computeConditionStatus(zone, {
        ...
        const temperatureRule = evaluateTemperatureRule(zone, [...recentReadings, prepared]);
        const createdAlerts = [];
        const createdNotifications = [];
    ...
    async moveStock(rawPayload, user) {
        ...
        const movementLotCode = String(lot.lot_number
            || lot.source_lot_internal
            || lot.source_lot_supplier
            || payload.lotCode
            || lotRef);
        ...
        if (destination) {
            const destinationStatus = String(destination.location_status || "free");
        ...
        if (source) {
            ...
            const nextLots = sourceAfterPalettes <= 0 && lotRef
                ...
                : currentLots;
            await Locations.updateOne({ id: source.id }, {
                $set: {
        ...
        if (destination) {
            await Locations.updateOne({ id: destination.id }, {
                $set: {
        ...
        if (movementLotId) {
            const StockLots = getCollectionModel("stock_lots");
            const lotUpdate = { updated_at: nowIso() };
            if (destination) {
    ...
    async findZoneById(id) {
        const Zones = getCollectionModel("storage_zones");
        return sanitizeDocument(await Zones.findOne(this.buildIdFilter(id)).lean().exec());
    }
    async findLocationById(id) {
        const Locations = getCollectionModel("storage_locations");
        return sanitizeDocument(await Locations.findOne(this.buildIdFilter(id)).lean().exec());
    }
