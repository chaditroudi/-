...
const locations = [];
const stockLots = [];
const receptionUnits = [];
const makeQuery = (value) => ({
    lean: () => ({
        exec: async () => value,
    }),
});
const matchesOrClause = (row, query) => {
    const clauses = Array.isArray(query.$or) ? query.$or : [query];
    return clauses.some((clause) => Object.entries(clause).every(([key, value]) => {
...
vi.mock("../../db/dynamic-model.js", () => ({
    ...
    getCollectionModel: (collection) => {
        if (collection === "stock_lots") {
            return {
                findOne: vi.fn((query) => makeQuery(stockLots.find((row) => matchesOrClause(row, query)) || null)),
        ...
        if (collection === "reception_units") {
            return {
                findOne: vi.fn((query) => makeQuery(receptionUnits.find((row) => matchesOrClause(row, query)) || null)),
            };
        }
        if (collection === "storage_locations") {
            return {
                findOne: vi.fn((query) => makeQuery(locations.find((row) => matchesOrClause(row, query)) || null)),
                find: vi.fn((query) => makeQuery(locations.filter((row) => matchesOrClause(row, query)))),
                updateOne: vi.fn((query, update) => {
...
describe("storageService.moveStock", () => {
    ...
    it("rejects unknown scanned LOT-ID values", async () => {
        await expect(storageService.moveStock({
            ...
            destinationLocationId: "dest-1",
            reason: "RECEPTION",
        }, { id: "user-1" })).rejects.toMatchObject({ code: "LOT_NOT_FOUND" });
    });
    it("rejects zero palette movements", async () => {
        await expect(storageService.moveStock({
            ...
            quantityPalettes: 0,
            reason: "RECEPTION",
        }, { id: "user-1" })).rejects.toMatchObject({ code: "MOVEMENT_PALETTE_QUANTITY_REQUIRED" });
    });
});
