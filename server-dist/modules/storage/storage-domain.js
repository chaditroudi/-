export const STORAGE_MOVEMENT_TYPES = [
    "ENTREE_ZONE",
    "SORTIE_ZONE",
    "TRANSFERT",
    "INVENTAIRE",
...
export const STORAGE_MOVEMENT_REASONS = [
    "RECEPTION",
    "FUMIGATION",
    "LAVAGE",
    "TRIAGE",
...
export const ROYAL_PALM_MODULE3_ZONES = [
    ...receptionZones.map((zone) => ({
        ...zone,
        name: `Zone Reception ${zone.code.slice(-2)}`,
        storage_family: "reception",
...
export const getZoneCodeFromLocationCode = (code) => {
    const match = String(code || "").trim().toUpperCase().match(LOCATION_CODE_PATTERN);
    return match?.[1] || null;
};
export const assertValidLocationCode = (code, expectedZoneCode) => {
...
export const buildLocationSeedsForZone = (zone, storageZoneId) => {
...
export const sortLocationsForSuggestion = (candidates, options = {}) => {
    ...
    return candidates
        ...
        if (leftVarietyMatch !== rightVarietyMatch)
            return leftVarietyMatch - rightVarietyMatch;
...
export const sortLotsForFefo = (lots) => [...lots].sort((left, right) => {
...
export const computeDlcAlertLevel = (date, now = new Date()) => {
...
export const evaluateTemperatureRule = (zone, readings, now = new Date()) => {
    if (zone.zone_type !== "cold_room") {
        return { level: "none", durationMinutes: 0 };
    }
    ...
    for (const reading of reversed) {
        if (typeof reading.temperature_c !== "number" || reading.temperature_c <= threshold)
            break;
    ...
    if (durationMinutes >= requiredMinutes) {
        return {
            level: threshold === 10 ? "critical_sms" : "red",
            durationMinutes,
        };
    }
    return { level: "none", durationMinutes };
};
export const evaluateDoorRule = (events, now = new Date()) => {
    ...
    const openingsLastHour = ordered.filter((event) => event.event_type === "OPEN" && (toTime(event.event_at) || 0) >= oneHourAgo).length;
    if (openingsLastHour > 15) {
        return { level: "responsable", openingsLastHour };
    }
    if (latest?.event_type === "OPEN") {
        ...
        if (openMinutes > 5) {
            return { level: "operator", openingsLastHour, openMinutes };
        }
    }
    return { level: "none", openingsLastHour };
};
export const isRawStorageDelayed = (location, now = new Date()) => {
    if (!String(location.zone_code || "").startsWith("SB-"))
        return false;
...
export const computeLocationStatus = (capacityPalettes, occupiedPalettes, currentStatus) => {
    if (currentStatus === "blocked")
        return "blocked";
    if (currentStatus === "reserved")
        return "reserved";
    if (occupiedPalettes <= 0)
        return "free";
    if (capacityPalettes > 0 && occupiedPalettes >= capacityPalettes)
        return "occupied";
    return "occupied";
};
