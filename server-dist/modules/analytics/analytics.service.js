...
const sumBy = (rows, ...keys) => rows.reduce((sum, row) => sum + readNumber(keys.map((key) => row[key]).find((value) => value != null), 0), 0);
...
const normalizePurchaseOrderInvoiceStatus = (status) => {
    switch (status) {
...
const normalizePurchaseOrderLineStatus = (status) => {
    switch (status) {
        case "partially_delivered":
        case "delivered":
        case "closed":
...
const getPurchaseOrderLineTargetQuantity = (line) => roundQuantity(readNumber(line.confirmed_quantity ?? line.quantity));
...
const deriveReceiptStatusFromLines = (lines) => {
    ...
    if (!hasReceipts)
        return "not_received";
    const hasToleranceReceipt = lines.some((line) => readNumber(line.received_quantity) > getPurchaseOrderLineTargetQuantity(line) + 0.0001);
...
const normalizePurchaseOrder = (order, lines) => {
...
const parseThreeWayMatch = (notes) => {
    if (!notes)
        return null;
...
const hasReceptionTrace = (lineNotes) => {
    if (!lineNotes)
        return false;
...
const periodStart = (period) => {
    ...
    if (period === "quarter")
        return shiftMonths(startOfMonthLocal(now), -3);
    return startOfMonthLocal(now);
};
const matchesDatePrefix = (value, dateKey) => readString(value).startsWith(dateKey);
const normalizeNotificationSeverity = (value) => {
    const raw = readString(value).toUpperCase();
    if (raw === "URGENT" || raw === "URGENCE")
        return "URGENCE";
    if (raw === "CRITICAL" || raw === "CRITIQUE")
        return "CRITIQUE";
    if (raw === "WARNING" || raw === "WARN" || raw === "AVERTISSEMENT")
...
const readLotQuantityKg = (row) => readNumber(row.quantity_kg ?? row.current_quantity ?? row.quantity ?? row.initial_quantity);
const readMovementQuantityKg = (row) => readNumber(row.quantity_kg ?? row.quantity ?? row.current_quantity);
const readMovementType = (row) => readString(row.type, row.movement_type).toUpperCase();
const readMovementDate = (row) => readString(row.created_at, row.movement_date, row.updated_at);
const readZoneCurrentKg = (row) => readNumber(row.current_weight_kg ?? row.current_load_kg ?? row.occupied_kg);
const readZoneTemperature = (row) => readNumber(row.temperature_celsius
    ?? row.current_temperature_c
    ?? row.temperature_c
    ?? row.current_temperature, Number.NaN);
const readZoneHumidity = (row) => readNumber(row.humidity_pct
    ?? row.current_humidity_percent
    ?? row.humidity_percent
    ?? row.current_humidity, Number.NaN);
const readReadingTemperature = (row) => readNumber(row.temperature_celsius ?? row.temperature_c, Number.NaN);
const readReadingHumidity = (row) => readNumber(row.humidity_pct ?? row.humidity_percent, Number.NaN);
const avgBy = (rows, key) => {
    const values = rows
...
const safeFind = async (collection, { filter = {}, select, sort, limit, } = {}) => {
    try {
...
const safeFindByDate = (collection, dateField, startIso, select, extraFilter = {}) => safeFind(collection, {
    filter: {
        ...extraFilter,
...
let AnalyticsService = class AnalyticsService {
    async getSuppliersMapByIds(ids) {
    ...
    async getSageOperations() {
        const [requisitions, orders, orderLines, receptions, receptionLots, productionOrders, stockLots,] = await Promise.all([
        ...
        const orderLineRows = orderLines;
        ...
        const poPendingInternalApproval = orderRows.filter((row) => row.status === "draft" && readNumber(row.total_amount) >= PURCHASE_ORDER_APPROVAL_THRESHOLD && !row.approved_by).length;
        const poReadyToSend = orderRows.filter((row) => row.status === "draft" && (readNumber(row.total_amount) < PURCHASE_ORDER_APPROVAL_THRESHOLD || Boolean(row.approved_by))).length;
        ...
        const receptionsWaitingQc = receptions.filter((row) => row.status === "EN_ATTENTE_QC" || row.status === "EN_QC").length;
    ...
    async getFounderLiveFactory() {
        ...
        const [fumActive, netActive, hydActive, triActive, pkgActive, alerts, fumToday, netToday, hydToday, triToday, pkgToday, receptions,] = await Promise.all([
    ...
    async getFounderSupplyFunnel(period) {
        const normalizedPeriod = normalizePeriod(period);
        const start = periodStart(normalizedPeriod).toISOString();
        const [receptions, fumigation, cleaning, hydration, triage, triageSublots, packagingOrders, palettes,] = await Promise.all([
    ...
    async getFounderPhase2Analytics(period) {
        const normalizedPeriod = normalizePeriod(period);
        const start = periodStart(normalizedPeriod).toISOString();
        ...
        const completed = (rows) => rows.filter((row) => readString(row.status).toUpperCase() === COMPLETED_STATUS);
        ...
        const gradeKg = { EXTRA: 0, CATEGORIE_I: 0, CATEGORIE_II: 0, REJETE: 0 };
        sublots.forEach((row) => {
    ...
    async getFounderPackagingAnalytics(period) {
        const normalizedPeriod = normalizePeriod(period);
        const start = periodStart(normalizedPeriod).toISOString();
    ...
    async getFounderAlertIntelligence(period) {
        const normalizedPeriod = normalizePeriod(period);
        const start = periodStart(normalizedPeriod).toISOString();
    ...
    async getFounderReceptionPhase1Analytics(period) {
        const normalizedPeriod = normalizePeriod(period);
        const start = periodStart(normalizedPeriod).toISOString();
        ...
        const sum = (rows, key, fallbackKey) => rows.reduce((total, row) => total + readNumber(row[key] ?? (fallbackKey ? row[fallbackKey] : undefined)), 0);
        const avg = (rows, key) => {
            const values = rows
                .map((row) => row[key])
    ...
    async getFounderStockStorageAnalytics(period) {
        const normalizedPeriod = normalizePeriod(period);
        const start = periodStart(normalizedPeriod).toISOString();
        ...
        const condAlerts = conditionReadings.filter((row) => {
            ...
            return ((Number.isFinite(temperature) && (temperature < 0 || temperature > 8)) ||
    ...
    async getFounderSupplierIntelligence(period) {
        const normalizedPeriod = normalizePeriod(period);
        const start = periodStart(normalizedPeriod).toISOString();
        ...
        receptions.forEach((row) => {
            ...
            if (row.qc_decision === "ACCEPTE")
                aggregation[supplierId].accepted += 1;
            else if (row.qc_decision === "REJETE")
                aggregation[supplierId].rejected += 1;
            else if (row.qc_decision === "QUARANTAINE")
                aggregation[supplierId].quarantine += 1;
        });
    ...
    async getFounderPlantHealthScore(period) {
        const normalizedPeriod = normalizePeriod(period);
        const start = periodStart(normalizedPeriod).toISOString();
        const [receptions, alerts, packagingOrders, ...phase2Collections] = await Promise.all([
            ...
            ...PHASE2_INPUT_COLLECTIONS.map((collection) => safeFindByDate(collection, "started_at", start, "weight_in_kg weight_out_kg", { status: COMPLETED_STATUS })),
        ]);
        const decided = receptions.filter((row) => Boolean(row.qc_decision));
