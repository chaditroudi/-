...
const computePhysicalLineStatus = (line) => {
...
const deriveReceiptStatus = (lines, receipts) => {
...
const deriveHeaderStatus = (order, receiptStatus) => {
    ...
    if (receiptStatus === "discrepancy")
        return "on_hold";
    if (receiptStatus === "delivered" || receiptStatus === "delivered_with_tolerance")
        return "delivered";
    if (receiptStatus === "partially_delivered")
        return "partially_delivered";
    return currentStatus;
};
const loadPurchaseOrder = async (purchaseOrderId) => {
    const order = sanitizeDocument(await PurchaseOrders().findOne({ id: purchaseOrderId }).lean().exec());
    if (!order) {
        throw notFound("PURCHASE_ORDER_NOT_FOUND", "Purchase order not found.");
    }
    const lines = sanitizeDocument(await PurchaseOrderLines()
...
const syncPurchaseOrderDerivedFields = async (purchaseOrderId) => {
    const { order, lines } = await loadPurchaseOrder(purchaseOrderId);
    const receipts = sanitizeDocument(await PurchaseOrderReceivingLots().find({ purchase_order_id: purchaseOrderId }).lean().exec());
    const receiptStatus = deriveReceiptStatus(lines, receipts);
    const nextStatus = deriveHeaderStatus(order, receiptStatus);
    const deliveredAt = nextStatus === "delivered" || nextStatus === "invoiced"
        ? String(order.delivered_at || new Date().toISOString())
        : null;
    const goodsReceiptCount = Array.from(new Set(receipts.map((receipt) => String(receipt.reception_id || "")).filter(Boolean))).length;
...
export const resolvePurchaseOrderReceiptTarget = async ({ purchaseOrderId, purchaseOrderLineId, supplierId, materialId, }) => {
    const { order, lines } = await loadPurchaseOrder(purchaseOrderId);
    const canonicalStatus = normalizeStatus(String(order.status || "draft"));
    if (!RECEIVABLE_STATUSES.has(canonicalStatus)) {
        throw badRequest("PURCHASE_ORDER_NOT_RECEIVABLE", `Purchase order ${order.order_number || purchaseOrderId} is not open for receipt.`);
    }
    if (String(order.supplier_id || "") !== String(supplierId || "")) {
        throw badRequest("PURCHASE_ORDER_SUPPLIER_MISMATCH", "The selected purchase order does not belong to the chosen supplier.");
    }
    const eligibleLines = lines.filter((line) => {
        if (normalizeLineStatus(line.line_status) === "cancelled")
            return false;
        const targetQuantity = getLineTargetQuantity(line);
    ...
    if (purchaseOrderLineId) {
        ...
        if (!targetLine) {
            throw badRequest("PURCHASE_ORDER_LINE_NOT_FOUND", "The selected purchase order line was not found on this order.");
        }
    }
    else if (materialId) {
        ...
        if (matchingLines.length === 1) {
            ...
            throw badRequest("PURCHASE_ORDER_LINE_REQUIRED", "Multiple order lines match this material. Please choose the exact purchase order line.");
        }
    }
    ...
    if (!targetLine) {
        throw badRequest("PURCHASE_ORDER_LINE_REQUIRED", "A purchase order line must be selected before receiving this delivery.");
    }
    return {
        order,
        line: targetLine,
...
export const registerReceptionAgainstPurchaseOrder = async ({ resolution, reception, lots, quantityTotal, actorId, }) => {
    ...
    if (nextReceived > maxReceivable + 0.0001) {
        throw badRequest("OVERDELIVERY_TOLERANCE_EXCEEDED", `The receipt exceeds the authorized quantity for order ${resolution.order.order_number || resolution.order.id}.`);
    }
    await PurchaseOrderLines().updateOne({ id: resolution.line.id }, {
        $set: {
    ...
    for (const lot of lots) {
        preparedReceipts.push(await prepareInsertDocument("purchase_order_receiving_lots", {
...
export const applyPurchaseOrderQcDecision = async ({ purchaseOrderId, receptionId, decision, }) => {
    const receipts = sanitizeDocument(await PurchaseOrderReceivingLots()
    ...
    const now = new Date().toISOString();
    const qcOutcome = decision === "ACCEPTE" ? "accepted" : decision === "REJETE" ? "rejected" : "conditional";
    for (const receipt of receipts) {
        const receivedQuantity = roundQuantity(readNumber(receipt.received_quantity));
        await PurchaseOrderReceivingLots().updateOne({ id: receipt.id }, {
            $set: {
    ...
    const groupedLineIds = Array.from(new Set(receipts.map((receipt) => String(receipt.purchase_order_line_id || "")).filter(Boolean)));
    for (const lineId of groupedLineIds) {
        const lineReceipts = sanitizeDocument(await PurchaseOrderReceivingLots().find({ purchase_order_line_id: lineId }).lean().exec());
        const acceptedQuantity = roundQuantity(lineReceipts.reduce((sum, receipt) => sum + readNumber(receipt.accepted_quantity), 0));
        const rejectedQuantity = roundQuantity(lineReceipts.reduce((sum, receipt) => sum + readNumber(receipt.rejected_quantity), 0));
        const existingLine = sanitizeDocument(await PurchaseOrderLines().findOne({ id: lineId }).lean().exec());
        if (!existingLine)
            continue;
