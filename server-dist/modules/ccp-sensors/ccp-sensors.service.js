...
import { signCcpReading } from "./ccp-signature.js";
const Devices = () => getCollectionModel("ccp_sensor_devices");
const simulatorSecret = () => process.env.CCP_SIMULATOR_SECRET || "royal-palm-ccp-simulator-dev";
...
let CcpSensorsService = class CcpSensorsService {
    phase2;
    storage;
    constructor(phase2, storage) {
        this.phase2 = phase2;
    ...
    async registerDevice(payload) {
        ...
        if (existing) {
            await Devices().updateOne({ device_code: deviceCode }, {
                $set: {
                    ...payload,
    ...
    async ingest(payload, headerSignature) {
        ...
        const device = sanitizeDocument(await Devices().findOne({ device_code: deviceCode }).lean().exec());
        if (!device)
            throw notFound("CCP_DEVICE_NOT_FOUND", "Unknown CCP sensor device.");
        if (String(device.status || "") !== "ACTIVE")
            throw unauthorized("CCP sensor device is not active.");
        const kind = String(payload.kind || device.kind || "").toUpperCase();
        ...
        const flag = kind === "FUMIGATION" ? (doorLocked ? 1 : 0) : 1;
        if (!targetRef || !Number.isFinite(value)) {
            throw badRequest("CCP_READING_INVALID", kind === "FUMIGATION"
        ...
        const existing = sanitizeDocument(await getCollectionModel(collection).findOne({ reading_key: readingKey }).lean().exec());
        if (existing)
            return { reading: existing, kind, replayed: true };
        ...
        return { reading: result.reading || result, kind, replayed: false, coldChain: result.coldChain };
    }
    async listDevices() {
...
CcpSensorsService = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [Phase2Service,
        StorageService])
], CcpSensorsService);
export { CcpSensorsService };
