...
let Phase2Service = class Phase2Service {
    ...
    async signFumigationCycle(id, payload) {
        ...
        if (refreshed?.operator_signed_at && refreshed?.quality_signed_at) {
            const readings = sanitizeDocument(await FumigationSensorReadings().find({ cycle_id: id }).sort({ read_at: 1 }).lean().exec());
            const protocol = String(refreshed.protocol || "");
            ...
            await FumigationCycles().updateOne({ id }, {
                $set: {
                    status: "TERMINE",
    ...
    async addFumigationSensorReading(payload) {
        ...
        const liveCycle = sanitizeDocument(await FumigationCycles().findOne({ id: cycleId }).lean().exec());
    ...
    async closeTriageSession(id) {
        ...
        if (parentWeightKg <= 0) {
            throw badRequest("TRIAGE_PARENT_WEIGHT_REQUIRED", "Le poids parent est requis pour clôturer le triage avec un bilan matière.");
        }
        ...
        const parentLot = parentLotId
            ? sanitizeDocument(await getCollectionModel("reception_lots").findOne({ id: parentLotId }).lean().exec())
            : null;
        ...
        const costByGrade = Object.fromEntries(costAllocation.streams.map((stream) => [stream.id, stream]));
        await TriageSessions().updateOne({ id }, {
            $set: {
                status: "TERMINE",
                ended_at: now,
