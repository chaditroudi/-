...
const rs = (v) => (v != null ? String(v) : '');
const rn = (v) => (typeof v === 'number' ? v : null);
export function buildGenealogyFromDossier(dossier) {
    const nodes = [];
    const edges = [];
    const colCounts = {};
    let edgeSeq = 0;
    ...
    const addEdge = (from, to, edgeType = 'process') => {
        if (!from || !to || from === to)
            return;
        edges.push({ id: `e${edgeSeq++}`, from, to, edgeType });
    };
    // ── Reception lot ──────────────────────────────────────────────────────────
    const receptionLots = dossier.reception_lots ?? [];
    const mainLot = receptionLots[0];
    ...
    // ── QC inspections ─────────────────────────────────────────────────────────
    const qcInspections = dossier.qc_inspections ?? [];
    ...
    // ── Fumigation cycles ──────────────────────────────────────────────────────
    const fumCycles = dossier.fumigation_cycles ?? [];
    const fumIds = [];
    ...
    // ── Cleaning cycles ────────────────────────────────────────────────────────
    const clnCycles = dossier.cleaning_cycles ?? [];
    const clnIds = [];
    ...
    // ── Hydration cycles ───────────────────────────────────────────────────────
    const hydCycles = dossier.hydration_cycles ?? [];
    const hydIds = [];
    ...
    // ── Triage sessions ────────────────────────────────────────────────────────
    const triSessions = dossier.triage_sessions ?? [];
    const triIdMap = new Map(); // session_number → id
    // Determine what connects INTO triage
    const preTriageIds = [...fumIds, ...clnIds, ...hydIds];
    for (const tri of triSessions) {
        const id = rs(tri.id);
        if (tri.session_number)
            triIdMap.set(rs(tri.session_number), id);
        addNode({
            id,
            type: 'triage',
        ...
        if (preTriageIds.length > 0) {
            for (const pid of preTriageIds)
                addEdge(pid, id, 'process');
        }
        else {
            addEdge(mainLotId, id, 'process');
        }
    ...
    // ── Sub-lots ───────────────────────────────────────────────────────────────
    const subLots = dossier.sub_lots ?? [];
    const subLotIdByLotNumber = new Map();
    for (const sl of subLots) {
        const id = rs(sl.id);
        const lotNum = rs(sl.lot_number);
        if (lotNum)
            subLotIdByLotNumber.set(lotNum, id);
        addNode({
            id,
            type: 'sub_lot',
    ...
    // ── Production orders ──────────────────────────────────────────────────────
    const productionOrders = dossier.production_orders ?? [];
    const productionAllocations = dossier.production_allocations ?? [];
    const prodOrderIds = [];
    for (const po of productionOrders) {
        ...
        const allocsForOrder = productionAllocations.filter((a) => rs(a.production_order_id) === id);
        const allocatedSubLotIds = allocsForOrder
            .map((a) => {
            const lotNum = rs(a.lot?.lot_internal) || rs(a.reception_lot_id);
            return subLotIdByLotNumber.get(lotNum) ?? '';
        })
            .filter(Boolean);
        if (allocatedSubLotIds.length > 0) {
            for (const slId of allocatedSubLotIds)
                addEdge(slId, id, 'process');
        }
        else if (subLots.length > 0) {
            // Fallback: connect from all sub-lots
            for (const sl of subLots)
                addEdge(rs(sl.id), id, 'process');
        }
        else {
            addEdge(mainLotId, id, 'process');
        }
    ...
    // ── Output lots (PF) ───────────────────────────────────────────────────────
    const outputLots = dossier.output_lots ?? [];
    const outputLotIds = [];
    ...
    // ── Stock lots ─────────────────────────────────────────────────────────────
    const stockLots = dossier.stock_lots ?? [];
    const stockLotIds = [];
    ...
    // ── Packaging orders ───────────────────────────────────────────────────────
    const packagingOrders = dossier.packaging_orders ?? [];
    const pkgOrderIds = [];
    ...
    // ── Palettes ───────────────────────────────────────────────────────────────
    const packagingPalettes = dossier.packaging_palettes ?? [];
    ...
    const shipmentLines = dossier.shipment_lines ?? [];
    ...
    const finalUnits = packagingOrders.reduce((sum, pko) => sum + (rn(pko.produced_units) ?? 0), 0);
    return {
