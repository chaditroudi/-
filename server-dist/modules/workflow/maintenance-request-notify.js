...
const actionUrlFor = (id) => `/?tab=maintenance&request=${encodeURIComponent(id)}`;
const resolveNextApproverIds = async (level, requesterDepartment) => {
    if (!level)
        return [];
...
export const notifyMaintenanceRequestTransition = async (request, decision, actorId) => {
    ...
    if (decision.notify.nextApproverLevel) {
        ...
        const targetUserIds = await resolveNextApproverIds(level, level === "dept_manager" ? department : null);
