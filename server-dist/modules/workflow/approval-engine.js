...
import { appendTransition, buildTransitionRecord, } from "./workflow-types.js";
...
export const normalizeStatus = (def, value) => {
    const raw = String(value || "").trim();
    if (!raw)
        return def.initialStatus;
    const upper = raw.toUpperCase();
    if (def.statuses.includes(upper))
        return upper;
    return def.legacyStatusMap[raw.toLowerCase()] ?? def.initialStatus;
};
const requiredLevels = (def, state) => {
...
const departmentForLevel = (def, level, state) => level.department?.(state) ?? def.requesterDepartment(state);
export const submitWorkflow = (def, state, actor, options) => {
...
export const cancelWorkflow = (def, state, actor, reason) => {
    const membership = assertActor(actor);
    const from = normalizeStatus(def, state.status);
 ...
 */
export const manualTransitionWorkflow = (def, state, actor, opts) => {
    const membership = assertActor(actor);
    const from = normalizeStatus(def, state.status);
