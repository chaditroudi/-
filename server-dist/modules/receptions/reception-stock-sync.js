...
const ensureProductHasBusinessId = async (productModel, product) => {
    if (!product)
        return null;
    if (product.id)
...
const resolveProductId = async () => {
    ...
    const product = (await Product.findOne({ code: DEFAULT_RAW_PRODUCT.code }).lean().exec()) ||
...
export const syncReceptionLotsToStock = async (lots, options) => {
    ...
    for (const rawLot of lots) {
        ...
        const payload = {
            ...
            unit: lot.unit || "kg",
            status: lot.stock_status === "STOCK_LIBERE"
                ? "VALIDATED"
            ...
            qc_validated_by: lot.stock_status === "STOCK_LIBERE"
                ? (options?.actorId ?? existing?.qc_validated_by ?? null)
                : (existing?.qc_validated_by ?? null),
            qc_validated_at: lot.stock_status === "STOCK_LIBERE"
...
export const removeReceptionLotsFromStock = async (receptionLotIds) => {
    if (receptionLotIds.length === 0)
        return;
    await StockLotModel().deleteMany({ reception_lot_id: { $in: receptionLotIds } }).exec();
};
export const loadReceptionLots = async (receptionId) => {
