...
const recalculateSupplierMetrics = async (supplierId, actorId) => {
    ...
    if (notifs.length > 0) {
        await Promise.all(notifs.map((n) => emitNotification({
...
let ReceptionsService = class ReceptionsService {
    ...
    async intake(payload, actor) {
        ...
        if (clientRequestId) {
            const existingReception = sanitizeDocument(await Receptions().findOne({ client_request_id: clientRequestId }).lean().exec());
            if (existingReception?.id) {
                const receptionId = String(existingReception.id);
                const existingLots = sanitizeDocument(await ReceptionLots().find({ reception_id: receptionId }).lean().exec());
        ...
        if ((grossReadingId || tareReadingId) && (!grossReadingId || !tareReadingId || lots.length !== 1)) {
            throw badRequest("DEVICE_WEIGHING_PAIR_REQUIRED", "Verified device intake requires gross and tare reading IDs and exactly one intake lot.");
        }
        const readingRows = grossReadingId && tareReadingId
            ? sanitizeDocument(await getCollectionModel("weighbridge_readings").find({
        ...
        const agreedPrice = Number(supplier.agreed_price_tnd_per_kg);
    ...
    async moveReceptionLotToStorage(payload, actor) {
        ...
        const stockLot = sanitizeDocument(await StockLots().findOne({ reception_lot_id: lotId }).lean().exec());
    ...
    async recordWeighing(lotId, payload, actor) {
        ...
        if (payload.reading_id) {
            deviceReading = sanitizeDocument(await getCollectionModel("weighbridge_readings")
                .findOne({ reading_id: String(payload.reading_id) })
                .lean()
                .exec());
            if (!deviceReading)
                throw notFound("WEIGHBRIDGE_READING_NOT_FOUND", "Weighbridge reading not found.");
            if (deviceReading.consumed)
                throw badRequest("WEIGHBRIDGE_READING_CONSUMED", "Weighbridge reading was already consumed.");
            ...
            if (deviceReading.calibration_valid_until
        ...
        if (deviceReading) {
            const claimed = await getCollectionModel("weighbridge_readings").updateOne({ id: deviceReading.id, consumed: { $ne: true } }, {
                $set: {
                    consumed: true,
        ...
        const allWeighings = sanitizeDocument(await WeighingRecords().find({ lot_id: lotId }).sort({ captured_at: -1, created_at: -1 }).lean().exec());
